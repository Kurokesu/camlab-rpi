...
def test_every_line_gets_prefix(classifier):
    ...
    out, lines, rest = mirror_lines(buf, classifier)
...
def test_terminal_mirror_stays_bare(classifier):
    buf = b"12:50:03 ERROR camlab: boom\n"
    out, lines, rest = mirror_lines(buf, classifier, priorities=False)
    assert out == buf
    assert lines == ["12:50:03 ERROR camlab: boom"]
    assert rest == b""


def test_partial_tail_waits_for_newline(classifier):
    out, lines, rest = mirror_lines(b"12:50:03 INFO camlab: half", classifier)
    assert (out, lines, rest) == (b"", [], b"12:50:03 INFO camlab: half")


def test_split_line_keeps_level(classifier):
    _out, _lines, rest = mirror_lines(b"12:50:03 ERROR camlab: sp", classifier)
    out, lines, rest = mirror_lines(rest + b"lit\n", classifier)
...
def test_mirror_keeps_undecodable_bytes(classifier):
    out, lines, _rest = mirror_lines(b"\xff raw byte\n", classifier)
