...
README walkthrough targets SD on a Pi 5, any boot storage works. CM5 eMMC boots fastest, ~15 s power-on to viewfinder vs ~21 s from SD on the test unit. NVMe was tested and brings no benefit: it boots ~1 s slower than eMMC (from the NVMe controller init the eMMC fast-path skips) and the app needs neither the capacity nor the bandwidth.
