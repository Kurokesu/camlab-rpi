...
#   2. Write the camtest managed block in /boot/firmware/config.txt for the
#      chosen sensor/port (via camtest.config_manager).
#   3. Install the scoped privilege shim: /usr/local/bin/camtest-apply +
#      /etc/sudoers.d/camtest (validated with visudo).
...
( cd "$REPO_DIR" && python3 -m camtest.config_manager set \
    --overlay "$SENSOR" --port "$PORT" "${opt_args[@]}" )

# Install the scoped privilege shim + sudoers rule.
log "Installing /usr/local/bin/camtest-apply"
sed -e "s|CAMTEST_REPO_DIR|$REPO_DIR|g" \
    "$REPO_DIR/scripts/camtest-apply.sh" > /usr/local/bin/camtest-apply
chmod 0755 /usr/local/bin/camtest-apply

log "Installing /etc/sudoers.d/camtest"
tmp_sudoers="$(mktemp)"
sed -e "s|CAMTEST_USER|$CAMTEST_USER|g" \
    "$REPO_DIR/deploy/camtest-sudoers" > "$tmp_sudoers"
if visudo -c -f "$tmp_sudoers" >/dev/null; then
    install -m 0440 "$tmp_sudoers" /etc/sudoers.d/camtest
    rm -f "$tmp_sudoers"
