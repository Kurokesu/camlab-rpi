...
# Stages config only. One-shot finalizer locks down on next boot after settle.
#
# Stages: trim, packages, data, overlay, swap, finalize
#
# Safe to re-run. Requires sudo. --revert unlocks on next reboot.
#
# Usage:
#   sudo scripts/setup/readonly.sh           # stage read-only, arm the finalizer
...
FINALIZE_SCRIPT="/usr/local/sbin/camlab-readonly-finalize"
...
# eat the boot files the finalizer writes. A writable root keeps trimming
stage_trim() {
    log "Stage: fstrim"
    if [ "$REVERT" -eq 1 ]; then
...
# Overlay config. Inert until finalizer locks in
stage_overlay() {
    ...
    # Stage overlay disabled for writable settle-boot. Finalizer clears token and reboots
...
# Finalizer + one-shot unit. Locks down on next boot after settle. Last stage
stage_finalize() {
    log "Stage: finalizer"
    if [ "$REVERT" -eq 1 ]; then
        systemctl disable "$ONESHOT_UNIT" >/dev/null 2>&1 || true
        rm -f "/etc/systemd/system/$ONESHOT_UNIT" "$FINALIZE_SCRIPT"
        systemctl daemon-reload 2>/dev/null || true
        log "removed finalizer + one-shot unit"
        return
    fi

    install -d -m 0755 /usr/local/sbin
    cat > "$FINALIZE_SCRIPT" <<'FINEOF'
#!/bin/bash
...
logger -t camlab-readonly "finalizer starting"
...
FINEOF
    ...
    log "installed finalizer $FINALIZE_SCRIPT"
...
if [ "$REVERT" -eq 1 ]; then
    ...
    log "Staged. The next reboot settles first-boot tasks, then the finalizer"
